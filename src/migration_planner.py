...
def suggest_migration_types(queue_info):
    ...
    if queue_info["type"] in ["quorum", "stream"]:
        # If already migrated, suggest the other type.
...
def print_migration_summary(migration_plan):
    """Prints a summary of the migration plan to the console."""
...
def main():
    parser = argparse.ArgumentParser(description="RabbitMQ Queue Migration Analyzer")
    parser.add_argument("--vhost", default="%2f", help="Virtual host (default: '%%2f')")
    parser.add_argument("--queue", help="Specify a queue name to analyze")
    parser.add_argument("--all", action="store_true", help="Analyze all queues in the specified vHost")
    parser.add_argument("--json", action="store_true", help="Output results in JSON format")
    args = parser.parse_args()

    if args.all:
        results = analyze_all_queues(args.vhost)
        if args.json:
        ...
        migration_plan = generate_migration_plan(args.vhost, args.queue)
        ...
        if args.json:
            ...
            print_migration_summary(migration_plan)
    else:
        # If no queue specified, prompt interactively
